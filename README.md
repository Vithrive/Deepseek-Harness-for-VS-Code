...
- **macOS 剪贴板修复（自动安装配套插件 `dsh-webview-clipboard`）**：修复 macOS 上面板内 ⌘C / ⌘V / ⌘X / ⌘A 失效的问题——按键本可到达 DSH 页面，但浏览器对这些按键的原生默认动作在「跨源 iframe 内嵌于 webview」这条链路上不会发生。插件在 DSH 页面内拦截组合键并改用 `document.execCommand('paste'/'copy'/'cut'/'selectAll')` 显式执行。仅在「被 Electron 应用内嵌 + macOS」时启用；普通浏览器打开 DSH、以及 Windows / Linux 上的行为完全不变（Windows 原生路径本就正常）。
