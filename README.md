...
1. 从 [GitHub Releases](https://github.com/Vithrive/Deepseek-Harness-for-VS-Code/releases/latest) 下载最新的 `.vsix` 安装包：
   ...
   ```
   https://github.com/Vithrive/Deepseek-Harness-for-VS-Code/releases/latest/download/deepseek-harness-vscode-0.2.10.vsix
   ```

2. 在 VS Code 中安装：

   ```bash
   code --install-extension deepseek-harness-vscode-0.2.10.vsix
   ```
...
- 面板顶部提供「刷新」「重启 dsh web」和「在浏览器中打开」按钮：「刷新」仅在服务离线时才重载（在线时只确认状态、不打断对话）；「重启」只重启**本窗口启动**的 dsh，不会误杀其他窗口正在使用的 dsh web
...
- `dev-<版本号>`：开发分支，命名与扩展版本号一致（历史 `dev-0.1.0`，当前 `dev-0.2.10`）
